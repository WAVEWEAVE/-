...
> 포토리얼 합성부터 빈티지 장르까지, 4가지 스타일을 제공합니다.
...
**스타일 1 (푸드 합성)** — 동물이 음식 형태로 변형:

- `seamlessly merged into the shape and texture of a golden toast loaf, body transformed into bread, fur becoming a warm toasted crust`
- `blended into the shape of a glazed donut, fur melting into the sugary surface`
- `transformed into a croissant shape, body curled into flaky pastry layers`
...
**스타일 1 — 푸드 합성** (고양이 식빵 계열)

```
a seamless photorealistic composite, studio product lighting, hyper-detailed fur and food texture blending, surreal humor, sharp focus throughout
```
...
> 💡 **스타일 선택 기준**
> | | 스타일 1 푸드 합성 | 스타일 2 라이프스타일 | 스타일 3 빈티지 | 스타일 4 신비 |
> |---|---|---|---|---|
> | 무드 | 귀엽고 초현실적 | 쿨하고 도시적 | 위트있고 복고적 | 드라마틱하고 캠피 |
> | 어울리는 상품 | 쿠션·머그컵 | T셔츠·폰케이스 | 포스터·에코백 | T셔츠·포스터 |
...
### 예시 1 — 🍞 고양이 식빵 합성 — 스타일 1 (웜골드)

```
A portrait of a chubby orange tabby cat with half-closed sleepy eyes and a round loaf-shaped body, seamlessly merged into the shape and texture of a golden toast loaf, body transformed into bread, fur becoming a warm toasted crust, rendered in a seamless photorealistic composite, studio product lighting, hyper-detailed fur and food texture blending, surreal humor, sharp focus throughout, warm golden tones with soft studio lighting, centered as one large portrait filling the frame. Print-ready, no text. --ar 1:1 --s 200
```
...
### 예시 2 — 🍩 강아지 도넛 합성 — 스타일 1 (쿨 화이트)

```
A portrait of a fluffy golden Cockapoo puppy with curly fur and round dark eyes, blended into the shape of a glazed pink donut, fur melting into the sugary frosted surface, sprinkles scattered around, rendered in a seamless photorealistic composite, studio product lighting, hyper-detailed fur and food texture blending, surreal humor, sharp focus throughout, bright clean white studio lighting, centered as one large portrait filling the frame. Print-ready, no text. --ar 1:1 --s 200
```
...
| `{scenario}` 의 음식만 (스타일 1) | 식빵→도넛→크루아상 푸드 합성 시리즈 |
...
2. **스타일 3에는 `--style raw` 필수.** 없으면 빈티지 인쇄 질감이 사라져요.
3. **스타일 1·2에는 `--style raw` 쓰지 마세요.** 포토리얼 품질이 딱딱해져요.
4. **다중 캐릭터(예시 7)는 수를 3개 이내로.** 4개 이상은 얼굴이 뭉개져요.
5. **`--s` 는 스타일마다 최적값이 달라요.** 포토리얼 150~200 / 빈티지 250~300.
6. **배경은 `clean white` 또는 단색.** 배경 제거 도구로 투명화하면 모든 상품에 바로 쓸 수 있어요.
...
> 💡 **상시판매형 추천 조합.** ① 스타일 1 푸드 합성 → 쿠션·머그컵에 강함. ② 스타일 3 빈티지 장르 → 포스터·에코백에 강함. ③ 스타일 4 드라마틱 신비 → T셔츠 베스트셀러 계열.
